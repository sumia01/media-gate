...
## Phase 9.7: Monitoring-aware media item status state machine ✅
→ See ADR-133
- [x] `sync.RecalcMediaItemStatus` is the single documented authority for `available`/`partial`/`missing`; series coverage now counts only monitored aired episodes (`EpisodeMonitor > SeasonMonitor > not monitored`, the monitor worker's own hierarchy) when the item is monitored — a series monitoring only future seasons with old seasons deleted is `available`, not stuck on `missing`
- [x] Unmonitored items keep all-aired disk-completeness semantics; zero coverage with unparseable-name files present stays `partial`; requested series with no files stay `requested`
- [x] Recalc fires on every input change: all three monitoring endpoints (response carries the fresh status), `SyncLibrary` per item whose files changed (previously only single-item resync recalced), and once per monitored item per monitor cycle so an episode airing flips `available` → `partial` without any other write
- [x] Recalc no-ops on unmatched items — it used to overwrite the `"new"` auto-match-queue marker on manual resync, silently excluding the item from `MatchLibrary`
- [x] `SyncLibrary`'s orphan pass no longer hard-deletes monitored items whose files all vanished from disk (was destroying monitors + metadata + episodes); they settle to `missing`/`available` via recalc
...
- [x] Accurate monitoring enables/disables, automatic queued grabs, changed-metadata diffs, and direct manual media actions
- [x] Transactional mutation/history boundaries, safe external-operation outcomes, and global/per-user watched visibility
- [x] Details/Activity tabs: keep media controls and cumulative requesters on Details; place the toggleable saved auto-download check list and activity log on Activity
- [x] Lazy diagnostic/history loading only for the active tab's expanded panels, with cursor pagination, live invalidation, and bounded history rendering
...
- [x] Poster thumbnails rendered only partially after a full re-match — **not** a TMDB rate limit: `downloadPoster` truncated the live file with `os.Create` and streamed into it while `PosterHandler` served that same path, so a request landing mid-write got a well-formed `200` containing a partial JPEG, which `max-age=86400` then pinned for a day. Every file on disk in production was verified complete (`FF D9`) with zero download failures logged, confirming a read-side defect. Fixed by installing posters atomically (temp file + `os.Rename`) and switching the endpoint to `Cache-Control: no-cache` + `ETag`, so correctness no longer depends on each caller building a cache-busted URL — `WatchedView` had none, making a poisoned entry there permanently unfixable. Poster download also moved ahead of the `updated_at` bump that the frontend uses as its cache key — see ADR-129
- [x] Library view flickered and couldn't be scrolled during a full re-match — every per-item `media.item_matched` SSE event refetched the whole list with `loading = true`, and the `v-if="loading"` branch unmounted the entire poster grid (collapsing page height and resetting scroll) once per matched item. Fixed by making SSE-driven refetches background (grid stays mounted, keyed patch updates only changed items), throttling them to 1/s with a guaranteed trailing call, and dropping out-of-order responses via a fetch sequence guard. Also: poster `@error` `display:none` was permanent across Vue element reuse (symmetric `@load` reset added), and leaving the page mid-match now asks a truthful `confirm()` — the match is a backend job and keeps running either way — see ADR-131
- [x] Series switched to future-seasons-only monitoring (old seasons deleted) stayed `missing` forever — status coverage ignored monitoring entirely, and no monitoring-change endpoint ever recalculated status. Fixed by making the status state machine monitoring-aware and recalculating on every input change (monitoring edits, library sync file changes, monitor cycle) — see ADR-133 / Phase 9.7
