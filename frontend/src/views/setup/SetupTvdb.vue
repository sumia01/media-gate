...
async function testConnection() {
  ...
  testing.value = true
  try {
    const { data } = await client.POST('/settings/test-tvdb', {
  ...
  try {
    if (apiKey.value.trim()) {
      const { error: err } = await client.PUT('/settings', {
        body: { tvdbApiKey: apiKey.value.trim() },
      })
      if (err) {
        error.value = 'Failed to save TVDB API key'
        return
      }
    }
    emit('next')
...
<template>
  <div class="space-y-5">
    <div>
      <h2 class="text-lg font-semibold text-gray-200">TVDB API Key</h2>
      <p class="text-sm text-gray-500 mt-1">
        TheTVDB provides additional TV show metadata. This is optional — TMDB is used as the primary source.
      </p>
    </div>
    ...
    <form class="space-y-4" @submit.prevent="handleSubmit">
      ...
      <div>
        <label for="tvdb-key" class="block text-sm font-medium text-gray-400 mb-1.5">API Key</label>
        <input
          id="tvdb-key"
          v-model="apiKey"
          type="text"
          ...
          placeholder="Your TVDB API key (optional)"
        />
