...
- Media details show deduplicated requester attribution, with separate whole-series, future-season, season, and episode intent. Later monitor changes attribute only scopes enabled by that user; repeat requests reuse the item and additively enable scopes.
...
Current migrations reach version 9: `0005` adds latest monitor snapshots, `0006` indexes timeline dates, `0007` adds nullable evaluated-input timestamps, `0008` adds scoped requester attribution, and `0009` separates title, whole-series, and future-season intent. Legacy snapshots keep unknown input freshness. Tests that rewind a migration version must also remove schema added after that version, not merely change the version row.
