...
- Preserve metadata-only indexer diagnostics and full rendered-error redaction, including quoted relative redirect URLs. Keep original error chains for `errors.Is`/`errors.As`, but never log the unwrapped original cause. URL redaction covers userinfo/query/fragment, not path-embedded credentials; see ADR-147.
...
| Library browsing | ADR-145. Local title/genre filtering relies on the complete library response, with genre options derived before title filtering. Any future pagination must preserve library-wide search and genre discovery. |
| Discover, people, and timeline | ADR-135/136/146. Media identity is `(source, mediaType, externalId)`; never guess cross-provider aliases by title. Person identity uses provider IDs: resolve TVDB people only through an explicit TMDB remote ID or the provider-returned exact unique name, never a client-supplied fallback after provider failure. Legacy ID-less TMDB credits may use exact name/profile matching; ambiguity returns not found. Keep actor Movies/Series as counted tabs with independent bounded render limits. Preserve bounded page/date/render windows, abort/stale-response guards, and scroll position. Empty timeline windows are not an end marker; initial/Today views center today. |
...
| Indexers and release selection | ADR-074, ADR-112, ADR-121, ADR-123, ADR-147. Keep `SanitizeYAML` before parsing provider definitions. Untagged releases fall back to English only when no language was detected; `multi` retains priority. Preferred release is a soft preference after profile ranking. |
