...
| Discover, people, and timeline | ADR-135/136/146. Media identity is `(source, mediaType, externalId)`; never guess cross-provider aliases by title. Person identity uses provider IDs: resolve TVDB people only through an explicit TMDB remote ID or the provider-returned exact unique name, never a client-supplied fallback after provider failure. Legacy ID-less TMDB credits may use exact name/profile matching; ambiguity returns not found. Preserve bounded page/date/render windows, abort/stale-response guards, and scroll position. Empty timeline windows are not an end marker; initial/Today views center today. |
