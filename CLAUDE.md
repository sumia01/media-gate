...
| `DEFAULTUSER_EMAIL` | `MEDIAGATE_DEFAULTUSER_EMAIL` | `DefaultUser.Email` | _(empty)_ | Bootstrap user email (used only when no users in DB) |
| `DEFAULTUSER_PASSWORD` | `MEDIAGATE_DEFAULTUSER_PASSWORD` | `DefaultUser.Password` | _(empty)_ | Bootstrap user password (used only when no users in DB) |
