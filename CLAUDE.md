...
│   │   ├── matching/    # Media matching service (TMDB/TVDB auto-match + manual, cached clients)
│   │   ├── crypto/      # AES-256-GCM at-rest encryption for sensitive settings
│   │   ├── safenet/     # SSRF protection (private-IP rejection, safe HTTP transport)
...
- **SSRF protection**: `backend/internal/safenet/` package provides `IsPrivateIP`, `SafeTransport`, `SafeClient`, and `CheckHost` — rejects connections to private/loopback/link-local IPs by resolving hostnames before dialing. Applied to settings connection tests (`TestQBit`, `TestFlareSolverr`) and download URL validation (`CreateDownload` checks http/https scheme). See ADR-075.
